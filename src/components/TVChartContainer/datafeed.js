...
export default {
  ...
  getBars: async (
    ...
    const since =
      moment.unix(periodParams.from).utc().format("YYYY-MM-DDTHH:mm:ss") + "Z";
    const till =
      moment.unix(periodParams.to).utc().format("YYYY-MM-DDTHH:mm:ss") + "Z";
    ...
    try {
      const data = await getCoinBars(since, till);
