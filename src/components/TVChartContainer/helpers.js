...
export async function makeApiRequest(path) {
  try {
    ...
    const response = await fetch(
      `https://min-api.cryptocompare.com/${path}${queryType}${process.env.REACT_APP_CRYPTOCOMPARE}`
    );
...
export async function getCoinBars(since, till) {
  try {
    let response = await axios.post(
      "https://graphql.bitquery.io",
      GET_COIN_BARS(since, till),
      {
        headers: {
